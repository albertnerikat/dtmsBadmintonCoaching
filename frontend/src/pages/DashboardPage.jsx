...
export default function DashboardPage() {
  ...
  function SessionCard({ s }) {
    const { visible, show, scheduleHide } = useBalloon();
    const st = s.attendance_stats;
    return (
      <div
        className="relative"
        onMouseEnter={st ? show : undefined}
        onMouseLeave={st ? scheduleHide : undefined}
      >
        <div
        ...
        {st && visible && (
          ...
          >
            <div className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Attendance</div>
            <div className="space-y-1 text-sm">
              <div className="flex justify-between gap-8">
                <span className="text-gray-600">Total</span>
                <span className="font-semibold">{st.total}</span>
              </div>
              <div className="flex justify-between gap-8">
                <span className="text-gray-600">Present</span>
                <span className="font-semibold text-green-700">{st.present}</span>
              </div>
              <div className="flex justify-between gap-8">
                <span className="text-gray-600">Free</span>
                <span className="font-semibold text-yellow-600">{st.free}</span>
              </div>
              <div className="flex justify-between gap-8">
                <span className="text-gray-600">Absent</span>
                <span className="font-semibold text-red-600">{st.absent}</span>
              </div>
            </div>
          </div>
        )}
